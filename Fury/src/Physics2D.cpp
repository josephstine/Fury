...
void Physics2D::Tick(float dt){
	...
	if(entity->desiredHeading < entity->heading){
		std::cout<<"REEEEEEEE:"<<entity->desiredHeading<<std::endl;
		std::cout<<entity->heading<<std::endl;
	...
	entity->velocity.z = Ogre::Math::Cos(Ogre::Degree(entity->heading)) * (entity->speed); //adjacent/hyp
	entity->velocity.x = Ogre::Math::Sin(Ogre::Degree(entity->heading)) * -(entity->speed); //opposite/hyp
