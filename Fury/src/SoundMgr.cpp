...
void SoundMgr::initialize(void){
	...
	int audioId = 1;
	loadAudio("TankMoving.ogg", audioId);
	playAudio(audioId);
	...
	unsigned int sid;
        //background music
	std::string filename = "/home/josephstine/workspace/Fury/data/sounds/BackgroundMusic.wav";
...
bool SoundMgr::initWatercraftSounds(){
		...
		std::string selectionFilename = "/home/josephstine/workspace/Fury/data/sounds/TankMoving.wav";
        std::string selection2Filename = "/home/josephstine/workspace/Fury/data/sounds/TankMoving.wav";
...
void SoundMgr::syncListenerToCamera(){
	...
	this->velocity[1] = 0;
	this->velocity[2] = 0;
	alListener3f(AL_VELOCITY, 0, 0, 0); //http://www.ogre3d.org/tikiwiki/OpenAl+Soundmanager0 // or we will use
	...
	Ogre::Quaternion q = engine->gameMgr->cameraNode->getOrientation();
...
bool SoundMgr::registerSelection(Entity381 et, std::string filename){
    ...
    if (this->reserveAudio(filename, false, sid)){
                int lastIndex = -1;
                for (int i = 0; i < soundPerEnt; i++){
                    if (this->selectionSoundsDictionary[et.audioId][i] == -1){
                        lastIndex = i;
                        break;
                    }
                }
                ...
                this->selectionSoundsDictionary[et.audioId][lastIndex] = sid;
                alSourcei(this->sourceInfo[sid].source, AL_REFERENCE_DISTANCE, 2000.0f);
                alSourcei(this->sourceInfo[sid].source, AL_MAX_DISTANCE, 8000.0f);
 ...
 *
	...
	soundMgr -> loadAudio("TankMoving.ogg", &audioId, true);
 */
